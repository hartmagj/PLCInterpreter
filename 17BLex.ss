...
(define apply-env
  (lambda (env sym)
    (let ([helpVal (apply-env-help env sym)])
      (if (box? helpVal)
        (unbox helpVal)
        helpVal))))
  ...
  (lambda (rands env vars)
    (if (null? vars)
        ...
        (if (list? (1st vars))
            ...
            (cons (apply-env-help env (2nd (1st rands)) ; Need to write
                    )
  ...
  (lambda (express env)
    (let ([exp (syntax-expand express)])
      (cases expression exp
          ...
          (cases proc-val proc-value
            [closure-ref (vars body envir)
              (let ([args (eval-rands-ref rands env vars)])
                (apply-proc proc-value args))]
            [else 
              (let ([args (check-quote rator rands env)])
                (apply-proc proc-value args))]))]
        [lambda-exp (id body)
          (cond
            [(null? id)
        ...
        [if-exp (test iftrue)
        (if (equal? test '(var-exp else)) (eval-exp (syntax-expand iftrue) env)
          (if (eval-exp test env) (eval-exp (syntax-expand iftrue) env))
          )
        ]
        [if-else-exp (test iftrue iffalse)
          (if (eval-exp test env)
            (eval-exp (syntax-expand iftrue) env)
            (eval-exp (syntax-expand iffalse) env))]
        [begin-exp (body)
  ...
  (lambda (proc-value args)
    (cases proc-val proc-value
      ...
      [closure-ref (vars bodies env)
          (eval-let bodies (extend-env (map (lambda (x) (if (list? x) (2nd x) x)) vars) args env))]
...
(define (lexical-address datum)
  (let lex ((addresses '()) (datum datum))
    (cases expression datum
       ...
       [if-exp (test then) (if-no-else-exp (lex addresses test) (lex addresses then))]
